...
const (
	DefaultP2PPort  = 1850  // Default p2p port for listening
	DefaultHTTPPort = 18540 // Default TCP port for the HTTP RPC server
	DefaultWSPort   = 18550 // Default TCP port for the websocket RPC server
)
...
func DefaultDataDir() string {
	...
	if home != "" {
		switch runtime.GOOS {
		case "darwin":
			return filepath.Join(home, "Library", "Ecoterium")
		case "windows":
			// We used to put everything in %HOME%\AppData\Roaming, but this caused
			// problems with non-typical setups. If this fallback location exists and
			// is non-empty, use it, otherwise DTRT and check %LOCALAPPDATA%.
			fallback := filepath.Join(home, "AppData", "Roaming", "Ecoterium")
			appdata := windowsAppData()
			if appdata == "" || isNonEmptyDir(fallback) {
				return fallback
			}
			return filepath.Join(appdata, "Ecoterium")
		default:
			return filepath.Join(home, ".ecoterium")
		}
	}
