...
var (
	...
	TestNetGenesisTime = inter.Timestamp(1635541260 * time.Second)
	MainNetGenesisTime = inter.Timestamp(1635541200 * time.Second)
)
...
func GetTestNetValidators(num int) gpos.Validators {
	validators := make(gpos.Validators, 0, num)
...
func TestNetGenesisStore(num int, balance, stake *big.Int) *genesisstore.Store {
	genStore := genesisstore.NewMemStore()
	genStore.SetRules(opera.TestNetRules())

	validators := GetTestNetValidators(num)
